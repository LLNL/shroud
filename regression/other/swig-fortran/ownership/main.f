...
contains
  ...
  subroutine test_pod
    integer(C_INT), pointer :: intp1(:)
    type(SWIGTYPE_p_int) swig_p_int, p_cint
!!!    type(c_ptr) cptr
    integer(C_INT) lenvar
...
!!-
    ...
    lenvar = 42
!!    p_cint = cscalar_to_SWIGTYPE_p_int(lenvar)
    swig_p_int = returnintptrdimdefault(p_cint)
!!-    call assert_true(associated(intp1))
!!-    call assert_equals(7 , size(intp1))
