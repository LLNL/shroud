...
class TypeOut(util.WrapperMixin):
        ...
        for name in sorted(ns.keys()):
            ...
            if isinstance(nxt, dict):
                if nxt:
                    ...
                    output.append(1)
                    output.append(f"declarations: {name}")
    ...
    def write_class_types(self):
        ...
        if output:
            output = [
                f"# Types generated by Shroud for library '{self.newlibrary.library}'",
                "typemap:",
                1,
            ] + output
...
def dump_jsonfile(config, logdir, basename, newlibrary):
    ...
    """
    jsonpath = os.path.join(logdir, f"{basename}.json")
    fp = open(jsonpath, "w")
...
def dump_fmt(config, logdir, basename, newlibrary):
    """Write a JSON file for debugging.
    """
    jsonpath = os.path.join(logdir, f"{basename}.fmt.json")
    fp = open(jsonpath, "w")
...
def process_lang_wrapper_flags(options, wraplist):
    ...
    for opt in wraplist:
        if opt[:3] == "no-":
            options[f"wrap_{opt[3:]}"] = False
        else:
            options[f"wrap_{opt}"] = True
...
def main_with_args(args):
    ...
    if args.outdir_c_fortran and not os.path.isdir(args.outdir_c_fortran):
        raise SystemExit(
            f"outdir-fortran {args.outdir_c_fortran} does not exist"
        )
    if args.outdir_python and not os.path.isdir(args.outdir_python):
        raise SystemExit(
            f"outdir-python {args.outdir_python} does not exist"
        )
    if args.outdir_lua and not os.path.isdir(args.outdir_lua):
        raise SystemExit(f"outdir-lua {args.outdir_lua} does not exist")
    if args.outdir_yaml and not os.path.isdir(args.outdir_yaml):
        raise SystemExit(
            f"outdir-yaml {args.outdir_yaml} does not exist"
        )
    if args.logdir and not os.path.isdir(args.logdir):
        raise SystemExit(f"logdir {args.logdir} does not exist")
    ...
    logpath = os.path.join(args.logdir, f"{basename}.log")
    log = open(logpath, "w")
    ...
    for filename in args.filename:
        ...
        if ext in [".yaml", ".yml"]:
            ...
            log.write(f"Read yaml {os.path.basename(filename)}\n")
            fp = open(filename, "r")
...
#        dump_fmt(config, args.logdir, basename, newlibrary)
    ...
    if args.write_helpers:
        hfile = os.path.join(args.logdir, f"{args.write_helpers}.c")
