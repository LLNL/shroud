...
Shroud provides defaults for all names it needs to generate.  But to
allow user control of the names, Shroud provides several options and
format fields to control the defaults.

Format fields can be used to give a specific name for a symbol name.
Then there are options to provide a template of how a name should be
created from format field components.
A list of the available options are in the
:ref:`reference section <reference_option_templates>`

How Files names are created
---------------------------
...
in the current scope. The generated name must be *flattened* into a
unique symbol name.
Most names have a template option which may be used
...
.. code-block:: yaml
    ...
    options:
      F_name_impl_template: "{library}_{F_name_api}{function_suffix}"
...
Since Fortran is case insensitive, Shroud defaults to creating an
underscore name from mixed case names. For example, ``HelperFunction``
becomes ``helper_function``.

Shroud provides the options **F_API_case** and **C_API_case** to
control name creation.  The options can be set to *preserve*, *lower*,
*upper* or *underscore* to define the default behavior to mangle
names.
...
The generated C wrapper will use the *splicer* provided for the body:
